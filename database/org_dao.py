...
#get organization
def get_org(id: int, db: Session) -> Organization | None:
    statement = select(Organization).where(id == Organization.id)
    result = db.exec(statement).first()
    return result

def get_all_orgs(db: Session) -> dict:
...
def org_exists(owner_id: int, db: Session) -> bool:
    org = get_org_by_owner(owner_id, db)
    return org is not None

def get_org_by_owner(owner_id: int,
                    db: Session) -> Organization | None:
    try:
