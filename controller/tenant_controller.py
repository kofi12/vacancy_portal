from fastapi import Depends, status, APIRouter, UploadFile, Security
...
def tenant_create(user_data: TenantBase,
                db: Session = Depends(get_session), user: User = Security(get_current_user_with_scopes, scopes=["write:tenants"])):
...
def waitlist_tenant_create(user_data: TenantBase,
                db: Session = Depends(get_session), user: User = Security(get_current_user_with_scopes, scopes=["write:waitlist"])):
    return tenant_dao.create_waitlist_tenant(user_data, db)

@tenant_router.get('/tenant/{id}', tags=["Tenants"])
def tenant_get(t_id: int,
               db: Session = Depends(get_session), user: User = Security(get_current_user_with_scopes, scopes=["view:tenants"])):
...
def get_all_tenants(db: Session = Depends(get_session), user: User = Security(get_current_user_with_scopes, scopes=["view:tenants"])):
    return tenant_dao.get_tenants(db)

@tenant_router.get('/waitlist', tags=["Tenants"])
def get_waitlist(db: Session = Depends(get_session), user: User = Security(get_current_user_with_scopes, scopes=["view:waitlist"])):
    return tenant_dao.get_waitlist_tenants(db)

@tenant_router.post('/upload-pdf/{id}', tags=["Tenants"])
def upload(file: UploadFile, t_id: int, db: Session = Depends(get_session), user: User = Security(get_current_user_with_scopes, scopes=["write:documents"])):
    return upload_f(file, t_id, db)

@tenant_router.get('/download-pdf/{id}', tags=["Tenants"])
def download(t_id: int, db: Session = Depends(get_session), user: User = Security(get_current_user_with_scopes, scopes=["view:documents"])):
